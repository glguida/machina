...
WriteTypeCheck(FILE *file, register const argument_t *arg)
{
	...
	fprintf(file, "\tif (");
	if (!it->itIndefinite) {
	    fprintf(file, "(((mcn_msgtype_t*)&In%dP->%s%s)->msgt_inline != %s) ||\n\t    ",
...
WriteCheckArgSize(FILE *file, register const argument_t *arg)
{
	 ...
	 */
	fprintf(file, "(((mcn_msgtype_t*)&In%dP->%s%s)->msgt_inline) ? ",
		arg->argRequestPos,
		arg->argTTName,
		arg->argLongForm ? ".msgtl_header" : "");
    }
	...
	fprintf(file, "(((mcn_msgtype_t*)&(In%dP->%s))->%s)",
...
InArgMsgField(register const argument_t *arg)
{
    ...
    if (arg->argSuffix && !arg->argParent->argLongForm)
	sprintf(buffer, "((mcn_msgtype_t*)&(In%dP->%s))->%s",
...
WriteDestroyArg(FILE *file, register const argument_t *arg)
{
	...
	fprintf(file, "\tif (!((mcn_msgtype_t*)&In%dP->%s%s)->msgt_inline)\n",
		arg->argRequestPos,
		arg->argTTName,
		arg->argLongForm ? ".msgtl_header" : "");
...
WriteDestroyPortArg(FILE *file, register const argument_t *arg)
{
    ...
    if ((it->itInTrans != strNULL) &&
	(it->itOutName == MCN_MSGTYPE_PORTSEND))
    {
	fprintf(file, "\n");
...
WritePackArgValue(FILE *file, register const argument_t *arg)
{
	...
	if (it->itString) {
		...
		fprintf(file, "\tif (%sP != %s) {\n",
			arg->argVarName,
			arg->argVarName);
		fprintf(file, "\t\t((mcn_msgtype_t*)&OutP->%s%s)->msgt_inline = false;\n",
			arg->argTTName,
			arg->argLongForm ? ".msgtl_header" : "");
		if (arg->argDeallocate == d_YES)
		    fprintf(file, "\t\t((mcn_msgtype_t*)&OutP->%s%s)->msgt_deallocate = true;\n",
			    arg->argTTName,
			    arg->argLongForm ? ".msgtl_header" : "");
		else if (arg->argDeallocate == d_MAYBE)
		    fprintf(file, "\t\t((mcn_msgtype_t*)&OutP->%s%s)->msgt_deallocate = %s;\n",
			    arg->argTTName,
			    arg->argLongForm ? ".msgtl_header" : "",
...
WriteAdjustMsgCircular(FILE *file, register const argument_t *arg)
{
    ...
    if (arg->argType->itOutName == MCN_MSGTYPE_POLYMORPHIC)
	fprintf(file, "\tif (%s == MCN_MSGTYPE_PORTRECV)\n",
