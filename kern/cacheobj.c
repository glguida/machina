...
cacheobj_delmapping(struct cacheobj *cobj, struct cacheobj_mapping *cobjm)
{
  ...
  for (vaddr_t i = cobjm->start; i < cobjm->start + cobjm->size; i += PAGE_SIZE)
...
cacheobj_map(struct cacheobj *cobj, vmoff_t off, pfn_t pfn, bool roshared, vm_prot_t protmask)
{
  ...
  printf("CACHEOBJ: mapping object %p offset %lx with pfn %lx (roshared: %d prot: %x)\n", cobj, off, pfn, roshared, protmask);
  writelock(&cobj->lock);
  ...
  LIST_FOREACH (cobjm, &cobj->mappings, list) {
    ...
    printf("CACHEOBJ: mapping %lx to va %lx (%s)\n",
	   pfn, cobjm->start + off - obj_offstart, writable ? "writable" : "read-only");
    umap_map(cobjm->umap, cobjm->start + off - obj_offstart, pfn, HAL_PTE_P | HAL_PTE_U | (writable ? HAL_PTE_W : 0), NULL);
    umap_commit(cobjm->umap);
  }
#endif
  writeunlock(&cobj->lock);
